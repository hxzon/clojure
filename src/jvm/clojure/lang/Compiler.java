...
static Symbol resolveSymbol(Symbol sym){
    ...
    if(sym.ns != null)
        ...
        if(ns == null || ns.name.name == sym.ns)
            //如果有命名空间但是命名空间不存在（即类的静态成员），或者是本命名空间的符号（即非导入的符号）
            return sym;
    ...
    if(o == null)
        ...
        //如果符号对应到一个var（即该符号是别名），转化成该var的符号（定义该var时用的符号）
            {
            Var v = (Var) o;
...
//                                      " refers to:" + v);
            ...
            if(RT.booleanCast(RT.get(mm, arglistsKey)))
                ...
                //hxzon注意：sym的arglists元数据，加入到Var
                v.setMeta(vm);
                }
...
public static class AssignExpr implements Expr{
    ...
    public Object eval() {
        //InstanceFieldExpr,StaticFieldExpr,
        //VarExpr（设置本线程绑定值）,LocalBindingExpr（不支持）
...
static class InvokeExpr implements Expr{
    ...
    static public Expr parse(C context, ISeq form) {
        ...
        //除了#=，form.first() 总是符号，fexpr是VarExpr
...
public static class LetFnExpr implements Expr{
    ...
    static class Parser implements IParser{
        public Expr parse(C context, Object frm) {
            ...
            if(context == C.EVAL)
                //转成匿名立即调用函数：((fn* [] (letfn* [f1 .. f2 ..] body)))    ？
                return analyze(context, RT.list(RT.list(FNONCE, PersistentVector.EMPTY, form)));
