...
//其它tokon，包括nil，true，false，关键字，Symbol
...
//当命名空间存在时，例如::clojure.core/y
			else
				return null;
//当命名空间x不存在时，返回null（双冒号且带有命名空间限定时，要求命名空间已存在）
			}
		boolean isKeyword = s.charAt(0) == ':';
//以单个冒号开头的为关键字
		Symbol sym = Symbol.intern(s.substring(isKeyword ? 1 : 0));
//一个新的Symbol对象,new Symbol(xxx)
		if(isKeyword)
	...
	public Object invoke(Object reader, Object caret, Object opts, Object pendingForms) {
		...
		if(o instanceof IMeta)
			...
			if(o instanceof IReference)//对象是引用类型
				...
				return o;
//hxzon注意：引用类型直接替换掉原有的元数据，而序列类型则合并自身已有的元数据：^:x ^:y o
				}
			Object ometa = RT.meta(o);
//对象已携带的元数据，例如^:x ^:y o ，首先得到带有^:y 的 o ，现在合并^:x
